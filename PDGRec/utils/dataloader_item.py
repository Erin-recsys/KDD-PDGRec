...
class Dataloader_item_graph(DGLDataset):
    def __init__(self, app_id_path, genre_path,dataloader_steam):
        ...
        path_graph_single = os.path.join(base_dir, "data_exist/graph_single.bin")

        '''graph 1'''
        if os.path.exists(path_graph_single):
            self.graph_single,_ = dgl.load_graphs(path_graph_single)
            self.graph_single = self.graph_single[0]#load_graphs
            self.genre_single = self.build_edge_single(self.genre)#(torch.tensor(src), torch.tensor(dst))
            
            graph_data_single = {
                ('game', 'co_genre', 'game'): self.genre_single,#(torch.tensor(src), torch.tensor(dst))
            }
            graph_single=dgl.heterograph(graph_data_single)
            #graph_single.add_nodes(295)
            self.graph_single=graph_single
            dgl.save_graphs(path_graph_single,[self.graph_single])


    def build_edge_single(self, mapping):
        src = []
        dst = []
